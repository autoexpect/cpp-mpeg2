...
int main(int argc, char* argv[]) {
    if (argc < 2) {
        std::cerr << "Usage: " << argv[0] << " <output.ts>" << std::endl;
        return 1;
    }
    ...
    std::cout << "Added video stream with PID: 0x" << std::hex << video_pid << std::dec << std::endl;
    
    // 添加 AAC 音频流
    uint16_t audio_pid = muxer.AddStream(mpeg2::TSStreamType::AAC);
    std::cout << "Added audio stream with PID: 0x" << std::hex << audio_pid << std::dec << std::endl;
    ...
    for (int i = 0; i < 10; i++) {
        ...
        // 写入视频帧
        int ret = muxer.Write(video_pid, video_frame, timestamp, timestamp);
        if (ret != 0) {
        ...
        // 写入音频帧
        ret = muxer.Write(audio_pid, audio_frame, timestamp, timestamp);
        if (ret != 0) {
    ...
    std::cout << "TS muxing completed successfully" << std::endl;
    return 0;
}
