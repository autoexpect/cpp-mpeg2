...
int main(int argc, char *argv[])
{
    if (argc < 3)
    {
        std::cerr << "Usage: " << argv[0] << " <input.h264> <output.ps>" << std::endl;
        std::cerr << "  input.h264  - H.264 elementary stream with start codes (0x00 0x00 0x01)" << std::endl;
        std::cerr << "  output.ps   - Output MPEG-PS file" << std::endl;
        return 1;
    }
    ...
    std::cout << "Added H.264 video stream with SID: " << static_cast<int>(video_sid) << std::endl;
    ...
    SplitH264FrameWithStartCode(h264_data, [&](const std::vector<uint8_t> &nalu) -> bool
        ...
        if (IsH264VCLNalu(nalu_type)) {
            ...
            // 写入合并后的数据到 PS 流
            int ret = muxer.Write(video_sid, pending_nalus, pts, dts);
            if (ret != 0) {
            ...
            int ret = muxer.Write(video_sid, nalu, pts, dts);
            if (ret != 0) {
                std::cerr << "Failed to write NALU: " << ret << std::endl;
                return false;
            }
    ...
    std::cout << "\nPS muxing completed successfully" << std::endl;
